...
export default function UniversitySettingsHomePage() {
  const { locale } = useParams();
  const router = useRouter();
  const t = useTranslations("university_setting");
  ...
  const handleDeleteConfirm = async () => {
    setDeleting(true);
    try {
      const res = await fetch(`/api/university/delete`, {
        method: "DELETE",
      });
      ...
      <Paper sx={{ p: 4, borderRadius: 2, boxShadow: 3 }}>
        ...
        <List>
          {menuItems.map(({ id, label, description }) => (
            <Box key={id} mb={2}>
              <ListItemButton
                component={Link}
                href={`/${locale}/university/settings/${id}`}
                sx={{
                  borderRadius: 1,
        ...
        <Box mt={6} borderTop="1px solid #ddd" pt={4}>
          <Typography variant="h6" fontWeight="bold" color="error" gutterBottom>
            {t("delete_university_account")}
          </Typography>
          <Typography variant="body2" mb={2}>
            {t("delete_university_account_description")}
          </Typography>
          <Button
            variant="contained"
            color="error"
